...
class Instalacion {
    ...
    public function obtenerTodasPorCaja($id_caja) {
        $query = "SELECT * FROM tbl_instalaciones WHERE id_caja = :id_caja ORDER BY fecha_instalacion DESC";
    ...
    public function obtenerPorId($id_instalacion) {
        $query = "SELECT * FROM tbl_instalaciones WHERE id_instalacion = :id_instalacion LIMIT 1";
        $stmt = $this->conn->prepare($query);
