...
class UserModel extends Model {
    ...
    public function register($data) {
        ...
        try {
            ...
            $sql = "SELECT id_usuario FROM {$this->table} WHERE correo = :correo OR cedula_identidad = :cedula_identidad OR nombre_usuario = :nombre_usuario LIMIT 1";
            $stmt = $this->db->prepare($sql);
            $stmt->bindValue(':correo', $data['correo'], PDO::PARAM_STR);
            $stmt->bindValue(':cedula_identidad', $data['cedula_identidad'], PDO::PARAM_STR);
            $stmt->bindValue(':nombre_usuario', $data['nombre_usuario'], PDO::PARAM_STR);
            $stmt->execute();
            if ($stmt->rowCount() > 0) {
                return false; // Email, cédula o usuario ya existe
            }
    ...
    public function getByRole($role) {
        ...
        switch ($role) {
            case 'admin':
                $pattern = 'admin@solufiber-srl.com';
                $sql = "SELECT * FROM {$this->table} WHERE correo = :pattern ORDER BY nombre_completo ASC";
                break;
            case 'operator':
                $pattern = '%operador%@solufiber-srl.com';
                $sql = "SELECT * FROM {$this->table} WHERE correo LIKE :pattern AND correo NOT LIKE 'admin@solufiber-srl.com' ORDER BY nombre_completo ASC";
                break;
            case 'technician':
                $pattern = '%@solufiber-srl.com';
                $sql = "SELECT * FROM {$this->table} WHERE correo LIKE :pattern AND correo NOT LIKE 'admin@solufiber-srl.com' AND correo NOT LIKE 'operador%@solufiber-srl.com' ORDER BY nombre_completo ASC";
                break;
            case 'client':
                $sql = "SELECT * FROM {$this->table} WHERE correo NOT LIKE '%@solufiber-srl.com' ORDER BY nombre_completo ASC";
                break;
            default:
                return [];
        }
        ...
        $params = [];
        if (strpos($sql, ':pattern') !== false && isset($pattern)) {
